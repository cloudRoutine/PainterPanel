...
SetWorkingDir %A_ScriptDir%                             ; Makes the script use it's own directory as the working directory

Process, Priority, , High                               ; Set the process priority of the script to high

Argument1 = %1%                                         ; Not sure <-----------
        
        
RID_UsagePage       = 0x0d                              ; Digitizer HID
RID_Usage           = 0x04                              ; Touch Screen HID
        
        
ControlSource       =                                   ; .txt File with the same name will load as control definition
        
HideOnMouseCursor  := 1                                 ; Hide window if mouse cursor comes over the window
        
GuiSnapSpacing     := 15                                ; Window Snap Spacing
        
ExitKey             =                                   ; No hotkey for exit 
ReloadKey           =                                   ; No hotkey for reload
        
        
        
GoSub, CreateGui                                        ; Create GUI for the PainterPanel
...
GetDisplayOrientation()                                 ; Detect DisplayOrientation
                                                        ; Refer to http://msdn.microsoft.com/library/ms819437.aspx
{
    ...
    dmDisplayOrientation    := 0                        ; DM_DISPLAYORIENTATION = 0x00000080
...
ExitSub:                                                ; Exit process.
;If( WasTouch )
;{
;  RIC_SetTouchState(1)
;}

    keys := PressedKeys("Get")                          ; Release keys that is held down if exist.
    Loop, PARSE, keys, |
    {
...
CreateGui:                                              ; Create GUI Window.
    ...
    If (!FileExist(ControlSource))                      ; Check Control source file.
    {
